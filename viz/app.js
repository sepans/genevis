...
const dataSetProperties = {
    dataFiles: [
        {
            src: 'data/genes_pca.json',
            type: FILE_TYPES.DATA, 
            unfold: d => Object.keys(d['PC1']).map(dd => {
                return {
                    name: dd,
                    PC1: d['PC1'][dd],
                    PC2: d['PC2'][dd],
                    cluster: d['cluster'][dd]
                }
            })
        },
        /*
    ...
    xAccessor: d => d['PC1'],
    yAccessor: d => d['PC2'],
    colorAccessor: m => m ? m.cluster : null,
    imageAccessor: null,
    info1ElContent: (nodeMetaData) => 'cluster: ' + nodeMetaData.cluster,//+nodeMetaData.c + 'dbscan: '+nodeMetaData.dbscanc,
    info2ElContent: (nodeMetaData) => '', //nodeMetaData.touched,
    selectedItemInfo: (node, nodeMetaData) => 
        `<div class="select" __data_id="${getIdByIndex(node.index)}" __data_index="${node.index}">${nodeMetaData.name} dc: ${nodeMetaData.cluster}</div>`,
    metaColumnTypes: {
...
q.awaitAll((error, results) => {
        ...
        //console.log(allCoords.slice(0, 20))
...
function findArticleLocation() {
    const title = new URLSearchParams(window.location.search.replace('?', '')).get('title')
    console.log('findArticleLocation', title)
    d3.json('http://toymaker.local:8888/similars?title='+title, data => {
        console.log('toymaker data ', data.location)
        const pointGeo = threejsObjects.pointGeo
              points = threejsObjects.points
        const x = xScale(-data.location[0]);
        const y = yScale(-data.location[1]);
        const z = 3//Math.random() * 500
        const nn = data.nn.slice(1, 10).map(d => `<li>${d}</li>`).join('')

        console.log('title' , x, y)
        // pointGeo.vertices.push(new THREE.Vector3(x, y, z));
        

        // pointGeo.colors.push(new THREE.Color().setRGB(0, 0, 0));

        // pointGeo.verticesNeedUpdate = true
        // points.geometry.colorsNeedUpdate = true
        // pointGeo.elementsNeedUpdate = true;
        const you = document.getElementById('youarehere')
        you.style.top = h - y
        you.style.left = x + 4
        console.log(nn)
        you.innerHTML = `<span>${title}</span><ul>${nn}</ul>`
    })
...
function addPoints(dataPoints, metaData, idAccessor, xAccessor, yAccessor, colorAccessor) {
	...
	for (let i = 0; i < pointCount; i ++) {
	    ...
	    pointGeo.vertices.push(new THREE.Vector3(x, y, z));
	    if(i===0) {
            console.log('first data point ', dataPoints[i])
            console.log('x, y for above ', xAccessor(dataPoints[i]), yAccessor(dataPoints[i]))
            console.log('screen x, y for above ', x, y)
        }
...
function showMetaBox(index, pointPosition) {
    ...
    if(nodeMetaData) {
        ...
        titleEl.innerText = nodeMetaData ? nodeMetaData.name : ''
